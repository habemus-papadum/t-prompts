/**
 * Before Code View Component
 *
 * Renders the "before" version of the prompt with diff annotations.
 * Uses its own separate folding controller for independent state management.
 */
...
import { applyTransform_BeforeDiffOverlay } from '../transforms/beforeDiffOverlay';
...
/**
 * Before code view component interface
 */
export interface BeforeCodeView extends Component {
  // Text-specific data
  chunkIdToTopElements: Map<string, HTMLElement[]>; // chunkId → array of top-level DOM elements
}

/**
 * Build a BeforeCodeView component from widget data and metadata
 *
 * @param data - Widget data containing before IR chunks
...
export function buildBeforeCodeView(
  ...
  element.className = 'tp-output-container tp-before-output wrap';
  ...
  let state: TransformState = { element, chunks: chunkIdToTopElements, data: beforeData, metadata };
  ...
  state = applyTransform_BeforeDiffOverlay(state); // Add diff markers for before view
  ...
  // 5. Selection tracking with debouncing
  ...
  // 6. Keyboard handler for collapse / expand
  ...
  // 7. Create folding client to respond to controller events
  ...
  function handleStateReset(): void {
    ...
    console.log('Before view state reset');
  }
  ...
  // 10. Return component with operations
  return {
