...
def build_widgets() -> None:
    ...
    run_command(["pnpm", "--filter", "@t-prompts/widgets", "build:python"], "Building widgets with pnpm")
    ...
    if result.stdout.strip():
        console.print("[red]✗ ERROR:[/red] Widget build produced uncommitted changes!")
        console.print("\n[yellow]Changes in widgets/dist:[/yellow]")
        console.print(result.stdout)
        console.print("\n[yellow]Please run 'pnpm build' and commit the changes before releasing.[/yellow]")
        sys.exit(1)

    console.print("[green]✓[/green] Widget build is up-to-date")


def run_widget_linting() -> None:
    """Run widget linting checks with pnpm."""
    console.rule("[bold blue]Running Widget Linting Checks")
...
def run_widget_tests() -> None:
    ...
    run_command(["pnpm", "test"], "Running pnpm test")
...
def main() -> None:
    ...
    # Parse arguments
    parser = argparse.ArgumentParser(description="Automate the release process for t-prompts")
    parser.add_argument(
        "bump_level",
    ...
    # Show title
    title = Panel.fit(
        "[bold cyan]t-prompts Release Script[/bold cyan]\n\n"
